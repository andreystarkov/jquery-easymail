    ...
    function bullShit(count){
        ...
        "другой", "тот же", "тлен", "ништяки", "вероятно", "но хуй там", "боль", "какарды там", "заебись же",
        "вообще", "охуенно же", "вроде", "может", "возможно", "шляпа", "ни о чем"];
        var bullshit = "";
        var last, one, div, a;
    ...
    function generateAll(){
        ...
        var subHeaderText = ['Ну то, '+bullShit(rand(1,3))+'?', 'Дела ок или '+bullShit(rand(1,2))+'?', 'Как дела, или '+bullShit(rand(1,2))+'?', 'Ну как там '+bullShit(rand(1,2))+'?',
        'Идею запилишь, или '+bullShit(1)+'?','Рассказывай давай, '+bullShit(1)+'.', 'Или че там '+bullShit(1)+'?', 'Что там, '+bullShit(rand(1,3))+'?'];
        var buttonText = [ 'Ну и вот, '+bullShit(2)+'.', 'Так всё и было, '+bullShit(1)+'.', 'Кароче шлю, '+bullShit(2)+'.', 'Отправить, '+bullShit(2)+'.'];
        var nameField = [ 'Я этот, как его, '+bullShit(5)+"...", 'Ну тот я, который, '+bullShit(5)+'...', 'Я тот, который он, или '+bullShit(4)+'...'];
        ...
        $('.container h1').shuffleLetters({step: 10, fps: 35,callback:function(){
            $('.container h1').addClass('show animated pulse');
            $('.container h2').addClass('show').shuffleLetters({callback:function(){
                $('.container h2').addClass('animated pulse');
            }});
    ...
    $(function(){
          ...
          $('#btn-mail').click( function(){
              ...
              if (status == 'sent'){
                $('#btn-mail').html('Пару минут жди '+bullShit(1)+'!');
              }

              if (status == 'empty'){
                $('#btn-mail').html('Мало букв '+bullShit(2)+".");
                $("textarea[name$='text']").removeClass('animated shake').addClass('animated shake');
              }
         });
